...
export default function Login() {
  ...
  async function handleRegister(e) {
    ...
    const r = await fetch(`${SERVER}/api/auth/register`, {
      ...
      body: JSON.stringify({ username, password, registrationCode: code }),
    });
    const data = await r.json();
    setLoading(false);
    if (!r.ok) return setErr(typeof data.error === 'string' ? data.error : 'Registration failed');
      ...
      <div style={s.card}>
        ...
        {tab === 'signin' ? (
          <form onSubmit={handleSignIn}>
            ...
            <input style={s.input} value={username} onChange={e => setUsername(e.target.value)} autoComplete="username" required />
          ...
          <form onSubmit={handleRegister}>
            ...
            <input style={s.input} value={username} onChange={e => setUsername(e.target.value)} autoComplete="username" required />
            <label style={s.label}>Password</label>
            <input style={s.input} type="password" value={password} onChange={e => setPassword(e.target.value)} autoComplete="new-password" required />
