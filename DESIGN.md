...
- **Submarine map awareness via sonar** — submarines have `los=0` so they contribute nothing to faction fog-of-war, meaning a faction with only submarines would have no map visibility at all (can't even see terrain). Proposed solution: submarines passively "see" water hexes within their sonar range (4 hexes), revealing those hexes as water in the fog-of-war. Land hexes adjacent to that water remain visible as terrain outline only ("coast detected") but units/buildings on land are hidden. Land hexes beyond sonar range stay fully dark. Implementation approach: in `computeVisibility`, add a special pass for submarine units — instead of using `los` for the standard LOS sweep, add all water hexes within `sonar_range` (4) to the visible set directly, bypassing the normal LOS blocking check (sonar doesn't care about terrain line-of-sight). Land hexes within sonar range could be added to `scouted` (terrain known, units hidden) rather than `visible` (fully revealed). This cleanly separates submarine situational awareness from surface unit LOS.

- **Fog of war: "last known state" for previously seen hexes** — currently there are only two client display states: Visible and Dark. Need a third: **Scouted** (previously seen, not currently visible). Scouted hexes show terrain type, roads, buildings, and urban/settlement status as they appeared when last observed, with a gray/desaturated overlay to indicate stale intel. Units are NOT shown (they may have moved). Implementation notes:
  - The `scouted_hexes` table already exists (tracks `faction_id`, `hex_q`, `hex_r`, `last_scouted_turn`) but only records that a hex was seen — it does not store what was seen.
  - Add a `last_known_state JSONB` column to `scouted_hexes` to snapshot the hex at time of last observation. Snapshot should include: `terrain`, `has_road`, `has_urban`, `has_settlement`, `has_light_vegetation`, `has_heavy_vegetation`, `has_bridge`, `buildings` (array of `{type, current_hp}` at time of scouting).
  - When `markScouted` runs at end of each turn, load the current hex state for all newly-visible hexes and write the snapshot into `last_known_state`.
  - Client rendering: `GET /api/map/:gameId/hexes` already returns scouted hex positions. Extend it to also return `last_known_state` for scouted hexes. Client renders three layers: **Visible** = full color, live data; **Scouted** = desaturated/gray overlay, render from `last_known_state` (roads, buildings, terrain color), no units; **Dark** = solid black, no info.
  - Buildings shown at their last-seen HP — a factory that was at 15 HP when last seen shows as 15 HP even if it has since been damaged or destroyed. This is intentional: players must have current eyes on it to know its status.
  - **Scouted state is strictly per-faction (per-player), not shared.** Two allied players may have different last-known snapshots of the same hex depending on when each last had eyes on it. The `scouted_hexes` table is already keyed by `faction_id` — never aggregate or merge scouted state across factions. Allied vision sharing (future feature) should share *current* visibility only, not scouted memory.
